...
abstract class BaseAutomailerPeer {
	...
	public static function getMapBuilder()
	{
		if (self::$mapBuilder === null) {
			self::$mapBuilder = new AutomailerMapBuilder();
		}
	 ...
	 * @param      string $column The column name for current table. (i.e. AutomailerPeer::COLUMN_NAME).
	 * @return     string
	 */
	public static function alias($alias, $column)
	{
		return str_replace(AutomailerPeer::TABLE_NAME.'.', $alias.'.', $column);
	}
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		$criteria->setPrimaryTableName(AutomailerPeer::TABLE_NAME);
		...
		if (!$criteria->hasSelectClause()) {
			AutomailerPeer::addSelectColumns($criteria);
		}
	...
	public static function doSelectOne(Criteria $criteria, PropelPDO $con = null)
	{
		...
		$critcopy->setLimit(1);
		$objects = AutomailerPeer::doSelect($critcopy, $con);
		if ($objects) {
			return $objects[0];
		}
		return null;
	...
	{
		return AutomailerPeer::populateObjects(AutomailerPeer::doSelectStmt($criteria, $con));
	}
	/**
	 ...
	 *
	 * @param      Automailer $value A Automailer object.
	 * @param      string $key (optional) key to use for instance map (for performance boost if key was already calculated externally).
	 */
	public static function addInstanceToPool(Automailer $obj, $key = null)
	{
	 ...
	 *
	 * @param      mixed $value A Automailer object or a primary key value.
	 */
	public static function removeInstanceFromPool($value)
	{
		if (Propel::isInstancePoolingEnabled() && $value !== null) {
			if (is_object($value) && $value instanceof Automailer) {
				$key = (string) $value->getId();
			} elseif (is_scalar($value)) {
				// assume we've been passed a primary key
				$key = (string) $value;
			} else {
				$e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or Automailer object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value,true)));
				throw $e;
			}
	 ...
	 * @return     Automailer Found object or NULL if 1) no instance exists for specified key or 2) instance pooling has been disabled.
  ...
  static public function getUniqueColumnNames()
  {
	...
	{
		return AutomailerPeer::CLASS_DEFAULT;
	}

	/**
	 * Method perform an INSERT on the database, given a Automailer or Criteria object.
	 *
	 * @param      mixed $values Criteria or Automailer object containing data that is used to create the INSERT statement.
    ...
    foreach (sfMixer::getCallables('BaseAutomailerPeer:doInsert:pre') as $callable)
    {
      ...
      if (false !== $ret)
      {
		...
		if ($values instanceof Criteria) {
			...
			$criteria = $values->buildCriteria(); // build Criteria from Automailer object
		}

		if ($criteria->containsKey(AutomailerPeer::ID) && $criteria->keyContainsValue(AutomailerPeer::ID) ) {
	...
	/**
	 * Method perform an UPDATE on the database, given a Automailer or Criteria object.
	 *
	 * @param      mixed $values Criteria or Automailer object containing data that is used to create the UPDATE statement.
	...
	/**
	 * Method perform a DELETE on the database, given a Automailer or Criteria object OR a primary key value.
	 *
	 * @param      mixed $values Criteria or Automailer object or primary key or array of primary keys
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			...
			AutomailerPeer::clearInstancePool();

			// rename for clarity
			$criteria = clone $values;
		} elseif ($values instanceof Automailer) {
			// invalidate the cache for this single object
			AutomailerPeer::removeInstanceFromPool($values);
			...
			$criteria->add(AutomailerPeer::ID, (array) $values, Criteria::IN);

			foreach ((array) $values as $singleval) {
				// we can invalidate the cache for this single object
				AutomailerPeer::removeInstanceFromPool($singleval);
			}
		}
	...
	/**
	 * Validates all modified columns of given Automailer object.
	 * If parameter $columns is either a single column name or an array of column names
	 * than only those columns are validated.
	 *
	 * NOTICE: This does not apply to primary or foreign keys for now.
	 *
	 * @param      Automailer $obj The object to validate.
	 * @param      mixed $cols Column name or array of column names.
	 *
	 * @return     mixed TRUE if all columns are valid or the error message of the first invalid column.
	 */
	public static function doValidate(Automailer $obj, $cols = null)
	{
		$columns = array();

		if ($cols) {
			$dbMap = Propel::getDatabaseMap(AutomailerPeer::DATABASE_NAME);
			$tableMap = $dbMap->getTable(AutomailerPeer::TABLE_NAME);
	...
	public static function retrieveByPK($pk, PropelPDO $con = null)
	{
		...
		if (null !== ($obj = AutomailerPeer::getInstanceFromPool((string) $pk))) {
			return $obj;
		}
		...
		$criteria->add(AutomailerPeer::ID, $pk);

		$v = AutomailerPeer::doSelect($criteria, $con);
	...
	public static function retrieveByPKs($pks, PropelPDO $con = null)
	{
		...
		if (empty($pks)) {
			...
			$criteria = new Criteria(AutomailerPeer::DATABASE_NAME);
			$criteria->add(AutomailerPeer::ID, $pks, Criteria::IN);
...
//
// NOTE: This static code cannot call methods on the AutomailerPeer class, because it is not defined yet.
// If you need to use overridden methods, you can add this code to the bottom of the AutomailerPeer class:
//
// Propel::getDatabaseMap(AutomailerPeer::DATABASE_NAME)->addTableBuilder(AutomailerPeer::TABLE_NAME, AutomailerPeer::getMapBuilder());
//
// Doing so will effectively overwrite the registration below.

Propel::getDatabaseMap(BaseAutomailerPeer::DATABASE_NAME)->addTableBuilder(BaseAutomailerPeer::TABLE_NAME, BaseAutomailerPeer::getMapBuilder());
